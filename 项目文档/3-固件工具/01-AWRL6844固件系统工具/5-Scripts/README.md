...
- ✅ **🆕 串口监视器** - 实时监控COM3/COM4数据流，支持暂停/恢复/Hex显示
  - **COM3 (User UART)**: 烧录端口，也可监控固件调试输出
  - **COM4 (Auxiliary Data Port)**: 雷达数据输出端口（点云数据、检测结果等）
...
```bash
# 切换到工具目录
cd 项目文档/3-固件工具/01-AWRL6844固件系统工具/5-Scripts
...
| 📡 **串口监视** | COM3/COM4实时监控 | 查看设备输出，调试固件<br>COM3: 烧录端口<br>COM4: 雷达数据输出 |
...
3. **配置端口**（可选）
   - 点击"🔄 刷新端口"扫描可用COM口
   - 在下拉框选择COM3（烧录端口）和COM4（雷达数据输出）
   - **COM3 (User UART)** - 用于固件烧录
   - **COM4 (Auxiliary Data Port)** - 用于雷达数据输出（点云、检测结果等）
   - 点击"测试COM3/COM4"验证连接
   - 如端口被占用，点击"🔓 释放端口"
...
2. 独立窗口打开，自动连接串口
3. 实时显示接收到的数据（自动滚动）
   - **COM3监控**：可查看烧录过程和固件调试输出
   - **COM4监控**：可查看雷达数据输出（点云、检测结果等）
4. 可点击"⏸️ 暂停"停止更新
