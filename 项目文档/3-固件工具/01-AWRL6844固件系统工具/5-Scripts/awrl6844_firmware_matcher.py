        ...
        for config in self.config_files:
            ...
            if not required_check['has_all_required']:
                ...
                validation['fatal_errors'].append(
                    f"[ERROR] 缺少必需命令：{', '.join(required_check['missing_commands'])}"
                )
            ...
            if required_check['has_invalid_commands']:
                ...
                validation['p0_required_commands'] = False
                validation['fatal_errors'].append(
                    f"[ERROR] 使用无效命令：{', '.join(required_check['invalid_commands'])}"
                )
            ...
            if not encoding_check['is_ascii']:
                ...
                validation['warnings'].append(f"[WARN] UTF-8编码问题：{encoding_check['issue']}")

            if encoding_check['has_chinese']:
                score -= 500.0  # 严重惩罚：中文字符（从-1000降低）
                validation['p0_encoding'] = False
                validation['warnings'].append(f"[ERROR] 包含中文字符（字节{encoding_check['position']}）")

            if encoding_check['has_percent_comment']:
                # v4.0.1: %注释很常见，不扣分，仅提示
                validation['warnings'].append("[INFO] 使用%注释符")
            ...
            if antenna_check['uses_antGeometryCfg']:
                ...
                score += 10.0
                validation['warnings'].append("[INFO] 使用antGeometryCfg配置")
            ...
            if antenna_check['uses_manual_config']:
                ...
                if manual_completeness >= 4:
                    ...
                    validation['warnings'].append("[OK] 手动天线配置完整")
            ...
            if matched_keywords:
                validation['warnings'].append(
                    f"[OK] 关键词匹配：{', '.join(matched_keywords[:3])}"  # 只显示前3个
                )
            ...
            if self._is_same_demo_directory(firmware.path, config.path):
                ...
                validation['warnings'].append("[OK] 同一Demo目录")
            ...
            if param_check['frameCfg_match']:
                ...
                validation['warnings'].append(
                    f"[WARN] frameCfg不匹配：{param_check['frameCfg_diff']}"
                )
            ...
            if param_check['runtimeCalibCfg'] == 1:
                ...
                score -= 20.0
                validation['warnings'].append("[WARN] runtimeCalibCfg=0（禁用校准）")
            ...
            if param_check['lowPowerCfg'] == 1:
                ...
                score -= 10.0
                validation['warnings'].append("[WARN] lowPowerCfg=0（未启用低功耗）")
