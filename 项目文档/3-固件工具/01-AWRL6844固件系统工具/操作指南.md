...
1. Win+X → 设备管理器
2. 展开"端口(COM和LPT)"
3. 查找 `XDS110 Class Application/User UART (COM3)` ← **烧录端口（实测验证）**
4. 另有 `XDS110 Class Auxiliary Data Port (COM4)` ← **雷达数据输出端口（实测验证）**
...
--------  -----------
COM3      XDS110 Class Application/User UART    ← 烧录端口（实测可用）
COM4      XDS110 Class Auxiliary Data Port      ← 雷达数据输出端口
```

> **⚠️ 重要提示（实测结果）**：
> - **COM3 (User UART)** 用于固件烧录 ✅
> - **COM4 (Auxiliary Data Port)** 用于雷达数据输出（如点云数据、检测结果等）✅
> - 端口号（COM3/COM4）可能因系统而异，以设备管理器显示为准
> - 烧录时必须选择 `XDS110 Class Application/User UART` 类型的端口
...
```powershell
# 进入脚本目录
cd "D:\7.项目资料\Ti雷达项目\项目文档\3-固件工具\01-AWRL6844固件系统工具\5-Scripts"
...
```powershell
cd "D:\7.项目资料\Ti雷达项目\项目文档\3-固件工具\01-AWRL6844固件系统工具\3-Tools"
...
3. 确认COM端口正确（烧录使用COM3，数据输出查看COM4）
4. 按RESET按钮重新启动
5. 重新烧录固件

> **注意**：
> - 烧录操作使用COM3（User UART端口）
> - 雷达数据输出在COM4（Auxiliary Data Port）
> - HelloWorld程序的串口输出可能在COM3或COM4，具体取决于程序配置
