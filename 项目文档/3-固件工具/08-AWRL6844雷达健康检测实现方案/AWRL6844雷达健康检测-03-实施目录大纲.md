...
| 章节 | 必读参考 | 备用参考 |
|------|---------|--------|
| **第1章 环境搭建** | SDK安装文档、InCabin Demo User Guide、⭐ **Radar Academy: radar_data.html** | Migration索引(硬件对比)、TI校准指南 |
| **第2章 人存检测** | InCabin Demo源码、⭐ **Radar Academy: motion_presence_detection.html** | motion_detect.c |
| **第3章 姿态检测** | ⭐ **pose.c/h**、**数据集** | dpc.c |
| **第4章 跌倒检测** | ⭐ **fall_detection.html**、pose.c | Migration Part3(相位) |
| **第5章 杂波移除** | ⭐ **Point Cloud Tuning Guide**、**Parameter Tuning Guide**、**3D Detection Layer Tuning** | TI校准指南、AOA校正DPU |
| **第6章 背景学习** | ⭐ **Parameter Tuning Guide** (Reference Zone概念) | 自主设计扩展 |
| **第7章 区域划分** | ⭐ **zoneDefinitionGenTool.xlsx**、**Parameter Tuning Guide** Section 2.3.1 | cuboidDef格式参考 |
| **第8章 呼吸检测** | IWR6843 Vital Signs Demo | - |
| **第9章 人数统计** | ⭐ **3D_people_tracking实现指南**、**tracker_layer_tuning**、⭐ **Radar Academy: tracking.html** | people_counting_customization、Tracking_radar_targets |
| **第10章 综合测试** | ⭐ **数据集**、**CPD_SBR_Test_Results**、**实验案例** | 验收标准参考 |
