...
def main():
    ...
    if not detach_mode:
        ...
        if existing_procs:
            ...
            if response:
                print(f"🔄 正在关闭 {len(existing_procs)} 个旧进程...")
                for proc in existing_procs:
                    try:
                        proc.terminate()
                        proc.wait(timeout=3)
                        print(f"✅ 已关闭旧进程 (PID: {proc.pid})")
                    except Exception as e:
                        print(f"⚠️ 关闭进程 {proc.pid} 失败: {e}")
        ...
        # CREATE_NO_WINDOW = 0x08000000  # Windows专用标志
        # CREATE_NEW_PROCESS_GROUP = 0x00000200
        subprocess.Popen(
            [sys.executable, script_path, '--detach'],
