...
**日期**: 2026-01-08
**最后更新**: 2026-01-09 (🔴 问题33：需用串口终端手动验证)
**状态**: 🔴 等待验证 - 固件已烧录，需用串口终端(PuTTY)验证CLI通信
...
│ - 命令: chirpComnCfg, chirpTimingCfg (InCabin格式)        │
...
├────────────────────────────────────────────────────────────┤
│ - CLI库: 自定义简化CLI                                     │
│ - Banner: "AWRL6844 Health Detection Firmware"            │
│ - 命令: 自定义命令格式                                     │
│ - 响应: 自定义格式，SDK Visualizer无法识别                │
└────────────────────────────────────────────────────────────┘
```
...
```
🔴 不要使用 SDK Visualizer 发送配置！
   SDK Visualizer 只适用于标准 mmw_demo 格式固件

✅ 正确方法：使用普通串口终端手动发送命令

步骤：
1. 打开串口终端（PuTTY / Tera Term / RealTerm）
2. 连接 COM 端口，设置 115200, 8N1
3. 板子上电/复位
4. 应看到启动信息：
   ********************************************
   *   AWRL6844 Health Detection Firmware     *
   *   Version 1.0.0                          *
   ********************************************
   Type 'help' for available commands

5. 输入 help 查看支持的命令
6. 逐行发送配置命令（不是使用.cfg文件导入）
```

**验证固件是否正常工作**：

```
如果看到启动Banner = UART初始化成功 ✅
如果help命令有响应 = CLI正常工作 ✅
如果配置命令被接受 = 命令解析正常 ✅
```

**状态**: 🔴 需要使用串口终端手动验证，不能用SDK Visualizer
