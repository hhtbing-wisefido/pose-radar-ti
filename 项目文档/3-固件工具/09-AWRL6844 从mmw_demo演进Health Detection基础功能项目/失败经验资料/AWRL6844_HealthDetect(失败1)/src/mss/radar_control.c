...
typedef struct {
    ...
    SemaphoreHandle_t configSem;  /* FreeRTOS semaphore (migrated from BIOS) */
...
int32_t RadarControl_init(void)
{
    ...
    /* Create semaphore for config completion (FreeRTOS) */
    gRadarState.configSem = xSemaphoreCreateBinary();
    if (gRadarState.configSem == NULL) {
